...
var baseUrl = 'https://app.wnews.mjmobi.com'
...
function _get(url, s, f) {
  var getData= wxPromisify(wx.request)
  console.log(url, "-----promise-start---_get----");
  return getData({
    url: url,
    header: {
  ...
  console.log(url, "---promise-end-----_get----");
}
...
function _post(url, data, s, f) {
  var getData = wxPromisify(wx.request)
  console.log(url, "----promise_post--start-------");
  return getData({
    url: url,
    header: {
      'content-type': 'application/json',
  ...
  console.log(url, "----promise-end-----_post----");
}
